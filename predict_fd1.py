    ...
    with torch.no_grad():
        for tt in tqdm(data_loader):
            ...
            for nB in range(predictions.shape[0]):
                ...
                index = (channel_slice,) + tuple(indice[nB])                                                  
                # squeeze batch dimension and convert back to numpy array
                

                
                prediction = predictions[nB].cpu().numpy()
                if prediction_channel is not None:
...
#    num_workers = datasets_config.get('num_workers', 1)
    ...
    for test_loader in l_test_loaders:
        ...
        miou   = cm[1,1]/(cm[1,0]+cm[1,1]+cm[0,1])  
        logger.info(f'recall: {recall:.4f} precision: {precision:.4f} Fscore: {fscore :.4f} mIOU: {miou :.4f} ')
        recpreF1.append(np.array([recall,precision,fscore,miou, cm[1,1], cm[0,1], cm[1,0], cm[0,0]]))
        p_ids.append(Path(test_loader.dataset.file_path).stem)
    
    df = pd.DataFrame(data = recpreF1, index = p_ids, columns = ['recall','precision', 'F1','miou', 'TP', 'FP', 'FN','TN'])
    df.to_csv(str(model_fd/'metrics.csv'))

...
class MeanIoU:
    ...
    NOTE!!!! This class only support batch-size =1 
    """

    def __init__(self, skip_channels=(), ignore_index=None,sigmoid_normalization = False,pad_width = None, **kwargs):
        """
    ...
    def __call__(self, input, target):
        ...
        if target.dim() == 4:
            ...
            if self.sigmoid_normalization:
                ...
                targs.scatter_(1, target.unsqueeze(1).long(), 1.0)
                targs = targs[:,1:]                                
            else:
    ...
    def _jaccard_index(self, prediction, target):
        ...
        """
        return (1.0+torch.sum(prediction & target).float()) / (1.0 + torch.sum(prediction | target).float())

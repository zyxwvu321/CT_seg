...
# -*- coding: utf-8 -*-
# Author: Minjie
# generate h5 file, img+mask+weight(optional) for training
"""
...
logger = call_logger('../CTraw/gen_randcrop.log', 'GenData') #log file of data generating
fd_all = '../CTraw/48_ct' #raw data of CT
mask_h5 = '../CTraw/h5/h5_48' # mask file, h5 format
h5_fd  = '../data/h5_rs48' #　output data file in h5py
...
for patient_case in path_patient_ids:    
    ...
    if mask_flip[p_id]==0:
        ...
        labels1 = (labels_h5[::-1,:,:]==maskid[p_id]).astype('int') #肝动脉
    else:
        labels1 = (labels_h5==maskid[p_id]).astype('int') 
    ...
    if len(area_labelbw)>1:
        logger.info('warning: more than one region, with areas {}, select region size >=5'.format(area_labelbw))
...
#    # read mask by mha file  , as mha data mask is not clean, not used now.  
#    itkimage1 = sitk.ReadImage(str(fn1))
#    if mask_flip[p_id]==0:
#    
#        labels1 = (sitk.GetArrayFromImage(itkimage1)[::-1,:,:]==maskid[p_id]).astype('int') #肝动脉
#    else:
#        labels1 = (sitk.GetArrayFromImage(itkimage1)==maskid[p_id]).astype('int') 
    ...
    # Only crop volume region with mask>1
    ...
    for idx,fn in enumerate(flist): #iter all images
        ...
        with pydicom.dcmread(fn) as dc:
            ...
            wc = 80#int(dc.WindowCenter)  here use default wc,wh
    ...
    anno_yxz = np.transpose(labels,(1,2,0)) #H,W,D
    img_yxz  = np.transpose(raw_im,(2,3,1,0)) #H,W,D,C
    
    for chs in range(img_yxz.shape[2]):
    ...
    # divide vessels into segs by branch points. calc its mean thinkness property
    prop_np = list()
    prop_np_valid = list()
    ...
    skel_label_bw_map = map_label[skel_label_bw] #new id, remove invalid idx segs
    ...
    # find which skel seg the mask area belongs
    ...
    for idx in range(prop_np_valid.shape[0]):
        ...
        w_gain  = min(50.0,max(2.0,50.0/ pow(m_thick,2))) #manually set weight gain for each part
